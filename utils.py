...
def busca_endereco(dataFrame):
    ...
    if status == 'OK':
        ...
        logger.info(
            'The request for {lat},{lon} returned no results!'.format(
                lat=latitude, lon=longitude))
    else:
        raise GMAPSError(
            'Google Maps API returned {status}, for {lat},{lon}'.format(
                status=status, lat=latitude, lon=longitude))
...
def addressResolver(json, dataFrame):
    ...
    try:
        ...
        dataFrame['ESTADO'] = data.get(
            "administrative_area_level_1_short", None)
        dataFrame['CIDADE'] = data.get(
            "administrative_area_level_2_long", None)
...
def checkLatitude(latitude):
    ...
    if latitude >= -90 and latitude <= 90:
        ...
        raise LatitudeWrongValue(
            '{lat} is not a valid latitude'.format(
                lat=latitude))
...
def checkLongitude(longitude):
    ...
    if longitude >= -180 and longitude <= 180:
        ...
        raise LongitudeWrongValue(
            '{lon} is not a valid longitude'.format(
                lon=longitude))
